...
bam_path="/collignon/Tawdarous/atac_seq_lab/results/alignment/rmChrM_bam_files"

# Path to the directory where the flagstat results will be saved
output_dir="/collignon/Tawdarous/atac_seq_lab/results/alignment/rmChrM_bam_files/flagstat_rmChrM"
...
if [ $(ls "$bam_path"/*rmChrM.bam 2> /dev/null | wc -l) -eq 0 ]; then
...
for sorted_bam in "$bam_path"/*rmChrM.bam; do
