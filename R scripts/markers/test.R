...
up_data <- process_homer_data(homer_up_m1_annotated, all_genes, "Upregulated")
down_data <- process_homer_data(homer_down_m1_annotated, all_genes, "Downregulated")
...
peak_data <- peak_data %>%
  mutate(Gene.Name = factor(Gene.Name, levels = rev(all_genes_ordered)))
...
# Sauvegarder le graphique combiné
ggsave("ATAC_seq_peak_analysis_with_external_labels.png", combined_plot, width = 49, height = 15, dpi = 400)
